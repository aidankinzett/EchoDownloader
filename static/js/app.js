...
socket.on('downloading_hq', function (data) {
    if (data.downloading < 100) {
        document.getElementById(downloading_guid + "-determinate").style.width = data.downloading + '%';
    }
    if (data.downloading === 100 && data.converting < 100) {
        document.getElementById(downloading_guid + "-determinate").style.width = data.converting + '%';
        document.getElementById(downloading_guid + "-words").innerText = "Converting..."
    }
    if (data.download === 100 && data.converting === 100) {
        document.getElementById(downloading_guid + "-progress").style.display = "none";
        document.getElementById(downloading_guid + "-icon").innerText = "high-quality";
        document.getElementById(downloading_guid + "-words").innerText = "";
        Materialize.toast('Finished downloading ' + downloading_guid, 4000);
        downloading = false;
    }
})
